...
If you've done web development for any amount of time, there's a good chance you've had to work with tools like [Webpack](https://webpack.js.org), [Rollup](https://rollupjs.org), [Browserify](https://browserify.org/) and other module bundlers. But do you know why we need them?

In this post, we take a look at one of the core areas of JavaScript - the **module ecosystem** - and why tools like the ones mentioned above are _required_ (🥁).
...
And this is where the module system and the tools mentioned above come into play.
...
There are several different module formats: [ES6 Modules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules) (or **ESM**), [CommonJS](https://en.wikipedia.org/wiki/CommonJS) (or **CJS**), [AMD](http://requirejs.org/docs/whyamd.html) and [UMD](https://github.com/umdjs/umd).

**Why so many?**

As I mentioned earlier, there are different JavaScript runtime environments. We have the **browser** with its various implementations (Chrome, Firefox, Safari, etc), and then the **server**, with runtimes like [Node](https://nodejs.org/) and [Deno](https://deno.land/). Each has its own different set of concerns, as we saw earlier

For a long time, there wasn't a single, agreed-upon standard between those environments in terms of how to deal with modules. There were concurrent attempts at solving the problem (even JavaScript standards themselves are _async_ 🥁), and as a result the above module formats have emerged.

Eventually, module format was standardized by the [TC39](https://tc39.es/) committee, with **ESM** becoming the de-facto "future-proof" standard going forward. Along with **CommonJS**, it is the format you are most likely to come across and work with today.
...
In a Node environment, however, modules are loaded as **instances**. This means that each imported module instance in your app has its own state, not shared with any of the other instances of the same module, so you no longer have global module state.
...
**When to use which?** That really depends on the intended use and structure of the module. One approach is to `export default` the main functionality of a module, to denote its main intent. Then any supplemental functionality or data are treated as named exports.

### ES6 Modules (ESM)

**ESM** is the standard established and governed by [TC39](https://tc39.es/) committee. It is the modern standard for JavaScript modules going forward, and is supported natively in all major browsers, as well as newer versions of Node (15+). For Node 14 and below, ESM can be enabled with the help of 3rd party libraries (eg. [`esm`](https://github.com/standard-things/esm)).
...
CommonJS (or **CJS**) is a module system that originated in [Node](https://nodejs.org/). It has been the prevalent format in the Node ecosystem for many years, and is still used by many libraries today.
...
CommonJS uses `.js` and `.cjs` file extensions, and it _cannot_ be loaded directly in a browser. For that exist specific tools that allow us to run CommonJS code in a browser environment, such as the previously-mentioned [Browserify](https://browserify.org/), [Rollup](https://rollupjs.org/) and others.
...
```js
const baz = require(isFoo ? './foo.js' : './bar.js');
```

However, this has certain tradeoffs. For instance, JavaScript tools can't know what's going to be loaded ahead of time. This unfortunately makes CommonJS modules not compatible with tree-shaking. It's much less of a concern on the server - since you're not sending code to the client - and if we rememeber that the CJS module format originated in Node, then it starts to make sense.

## Interoperability

With all the differences between the module formats we've seen so far, and the different runtime environment gotchas, it's evident why there is such a strong need for a **single** uniform module specification going into the future. Now let us ask a question:

**"What happens if you want to use a CommonJS module inside an ES6 module? Or the other way around?"**

And the short answer is, it depends. Browsers don't support this natively - so tooling comes to the rescue once again.
...
We'll take a closer look at how some of this interoperability works in a separate post.

## Closing Thoughts

Before wrapping this up, let's consider this for a second - if there are so many issues with interoperability...

**"Why is it difficult to move away from CommonJS?"**

First, it's _everywhere_. It's still the main format used in Node, and has been for many years (Node only recently started supporting ESM). The NPM ecosystem is _huge_. And though many popular library authors have started shipping both ESM and CJS compatible code, there are still many packages out there that are exclusively CommonJS. This is especially true for legacy systems, and enterprise code, which may not have the luxury of being moved to an entirely different module system.

Second, it's a lengthy process. Lots of large (and popular) codebases and tools still use and heavily rely on it. So while the goal is still to move to a single unified format down the road (ESM), it is not an overnight change, and will take some time for the entire ecosystem to migrate to it.
