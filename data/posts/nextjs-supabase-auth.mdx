...
**NOTE:** `Supabase UI` was still in early development stage at the time of this writing, so the API and some features may still change in the full release.
...
For this tutorial, we'll be using [Next.js](https://nextjs.org/) and the [TailwindCSS](https://www.tailwindcss.com/) starter template :rocket:
...
Then, install both the **Supabase Client**, and the **Supabase UI** component library:
...
Our application will be pretty simple. The main `Home` page will show the different authentication screens (ie. Sign In, Sign Up, etc.), as well as the authentication status and the ability to sign out. We'll also have a `Profile` page to display the currently logged-in user's data, which needs to be a "protected" page (meaning that only authenticated users can access it). We will take a look at how to accomplish this both on client and server-side in **Next.js**.
