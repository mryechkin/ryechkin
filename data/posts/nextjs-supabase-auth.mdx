...
We pass our `supabase` client instance to it via the `supabaseClient` prop, and _voila!_ 🎉 Now we have a shiny new UI, with all of the required forms to handle the login flows. However, before we can fully test this, we'll need to setup a few more things.
...
Next, we need a way to tell the `Auth` component which screen to show as a result of the auth state change. For example, when a user signs out, we want our `Auth` component to show the **Sign In** screen again. We can do this by passing `view` prop to the `Auth` component, which tells it which authentication flow to show. The `event` received by `onAuthStateChanged` will dictate which view we should be showing. We'll see how this gets utilized for updating a user's password in a later step.
...
Here, we're setting the `view` to `UPDATE_PASSWORD` when `PASSWORD_RECOVERY` event is dispatched, and to `SIGN_IN` when user logs out, or user is updated in session. Now all that's left is to pass this value to `Auth` component. In our main page `pages/index.js`, update the following:
...
With everything in place, we're now ready to try out the authentication flows. Make sure that you have the environment variables setup as outlined earlier, and then run the app with `npm run dev` on `localhost:3000`.
...
Enter your email and password, and press the "Sign up" button. This will send a confirmation email to the address provided, with a link to complete the registration process. It uses the default email template, which you can update in Supabase Dashboard settings (under "Templates"):
...
<PostImage
  src={`/blog/nextjs-supabase-auth/settings-url.png`}
  caption="Supabase URL Settings"
  width={888}
  height={456}
/>

Keep in mind that this works for development, but will need to be updated for running in production environments. See [this](https://github.com/supabase/supabase/discussions/1514) GitHub discussion for more details.

Clicking the confirmation link will redirect you to the site URL (`http://localhost:3000`), along with an `access_token` and `type`, which is verified by the Supabase Client. It then sets the local session, and fires the `SIGNED_IN` event.
...
This also fires the `SIGNED_IN` event, which gets picked up by our `onAuthStateChanged` listener we had setup earlier. The `session` and `user` objects are also updated, which means that we now have access to that information in our app.
