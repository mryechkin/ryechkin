...
export default function Card({ item, isExternal }) {
  return (
    <Link href={item.href} passHref>
      <motion.a
        className="retro-shadow focus grid grid-flow-row grid-cols-4 grid-rows-2 mt-6 max-h-full border-2 border-pink-500 rounded-lg overflow-hidden overflow-ellipsis sm:grid-flow-col sm:grid-rows-1 sm:max-h-48"
        whileFocus={{ y: -3 }}
        whileHover={{ y: -3 }}
        whileTap={{ y: 0 }}
      >
