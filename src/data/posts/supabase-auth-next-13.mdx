...
First things first, we'll need to update [Supabase](https://github.com/supabase/supabase-js) to the latest version (**v2**), and install [Formik](https://github.com/jaredpalmer/formik) (+ related dependencies) and the [Next.js Auth Helpers](https://github.com/supabase/auth-helpers):

```bash
npm install @supabase/supabase-js@latest @supabase/auth-helpers-nextjs classnames formik yup
```
...
This is an authenticated-only **Profile** page, showing some basic user info for the current user. If no session is found, it will redirect to `/`.
...
Note how we need to include `<html>` and `<body>` tags here as well. If you'd like to think of it in terms of the old `pages` directory, the root `layout.js` essentially combines the concepts of `_app.js` and `document.js` together into one.
