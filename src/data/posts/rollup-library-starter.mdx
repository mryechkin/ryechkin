...
```sh
npm i -D rollup rollup-plugin-analyzer rollup-plugin-terser @rollup/plugin-alias @rollup/plugin-babel @rollup/plugin-commonjs @rollup/plugin-image @rollup/plugin-node-resolve @babel/plugin-transform-runtime @babel/preset-env @babel/preset-react
```
...
This next one will help us reduce final bundle size by minifying the generated code. It's called [rollup-plugin-terser](https://github.com/TrySound/rollup-plugin-terser) and uses [terser](https://github.com/terser/terser) under the hood to minify the code.
...
Lastly, we have [rollup-plugin-analyzer](https://github.com/doesdev/rollup-plugin-analyzer). This plugin will print out some useful info about our generated bundle upon sucessfull builds:
...
And with that, we're done configuring Rollup! The full configuration file can be found [here](https://github.com/mryechkin/rollup-library-starter/blob/main/rollup.config.js) for your reference.
...
In order to ensure maximum interoperability with tools like Webpack (and others), we should also specify the [subpath exports](https://nodejs.org/api/packages.html#subpath-exports) separtely for each module format.
...
<PostImage
  caption="Rollup bundle analyzer plugin output"
...
>   [https://github.com/mryechkin/rollup-library-starter](https://github.com/mryechkin/rollup-library-starter)
> </div>
