...
<Warning>
  We need to use `preserveDirectives.default()` instead of just `preserveDirectives()`
  here, due to an
  [issue](https://github.com/Ephem/rollup-plugin-preserve-directives/issues/1) with the
  plugin in ESM environments.
</Warning>
