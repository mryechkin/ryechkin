...
When the browser loads that page, your application is initalized and React renders it into that `root` element using React DOM. From there, your application runs on the client side (meaning in the browser). This includes things like routing when using libraries like [React Router](https://reactrouter.com/).
...
With **Server Components**, a server can do a lot of that work before sending a response to the browser - and have React render everything on the **server**. The browser then gets just the resulting HTML, which it knows exactly what to do with.
...
When a React app runs on the client, the first thing a user will see is a blank screen. That's because the initial HTML is just an empty `<div>` element, and React hasn't had a chance to render anything yet. Only once the browser downloads the JS bundle and React initializes the app, do we see a result on the screen. Depending on the network speed, application bundle size, and other factors, this may take a varying amount of time as well. That's not the greatest user experience.

Server-Side Rendering (or **SSR**) is a way to address this problem. It is a mechanism through which a server can "pre-render" React components before a payload is sent to the browser, and send the rendered result as static HTML - which the browser can readily use. The catch is, in order for these components to be fully interactive and work as expected, they need to be "hydrated" on the client - and so the server still needs to send an additional payload alongside the HTML for it to fully work. Additionally, SSR only happens on the initial page load.

Prior to **RSC**, all React components were considered "client" components, because they all run in the browser (ie. the "client"). So even though SSR gives us pre-rendering of the static HTML, our React components aren't truly finished rendering until they've run on the client.

With Server Components, that all changes. **RSC** run _entirely on the server_, and their final rendered result is sent to the browser as HTML. There is nothing left to hydrate!

Not only that, but since RSC are run on the server, they allow us to execute server-only code right there in body of the main rendering function:
...
**Client Components** are just your regular React components we all use and love - they're essentially what all React components were prior to the introduction of **RSC**. The name "Client Component" is simply to distinguish them from the new **Server Components**, and doesn't introduce anything new.
...
- Enables [Streaming](https://nextjs.org/docs/app/building-your-application/rendering/server-components#streaming) and React [Suspense](https://react.dev/reference/react/Suspense)
