...
<Warning>
  Note that this guide is now outdated, and is using an older version of the **Next.js
  Auth Helpers** (pinned at `0.6.1`). Please see the [updated
  guide](/blog/supabase-auth-next-13-pkce) for the latest version that utilizes the
...
In October 2022, Next.js team announced [Next 13](https://nextjs.org/blog/next-13), and with it came the new [`app`](https://beta.nextjs.org/docs/app-directory-roadmap) directory architecture.
...
> Make sure to [read more](https://nextjs.org/docs/app/building-your-application/routing#the-app-directory) about the new `app` directory if you aren't familiar with the new features.
...
<Warning>
  The instructions below are using `@supabase/auth-helpers-nextjs@0.6.1`. At the time of
  writing, there is an issue with password reset flow in the latest version. Additionally,
  some of the methods used in this guide have been renamed in version
  [`0.7.0`](https://github.com/supabase/auth-helpers/releases/tag/%40supabase%2Fauth-helpers-nextjs%400.7.0).
  See the project repo in [GitHub](https://github.com/mryechkin/nextjs-supabase-auth) for
  any future changes.
</Warning>
...
```jsx filename="src/lib/supabase-browser.js"
...
```jsx filename="src/lib/supabase-server.js"
...
> Read more on this in ["moving Client Components to the leaves"](https://nextjs.org/docs/getting-started/react-essentials#moving-client-components-to-the-leaves) in Next.js docs for additional information.
...
```jsx filename="src/components/AuthProvider.jsx"
useEffect(() => {
  // ...
  const {
...
```jsx filename="src/components/Auth/index.js"
'use client';
...
<ImageCard src={`/blog/supabase-auth-next-13/webpack-error.png`} alt="Home page: Error" />
...
```jsx filename="src/app/profile/page.js"
import Link from 'next/link';
...
<ImageCard src={`/blog/supabase-auth-next-13/profile-1.png`} alt="Profile" />

Everything works great!

But let's say we want to add a "Sign Out" button:

<ImageCard src={`/blog/supabase-auth-next-13/profile-2.png`} alt="Profile" />
...
```jsx filename="src/components/SignOut.js"
'use client';
...
```jsx filename="src/app/profile/loading.js"
...
<ImageCard src={`/blog/supabase-auth-next-13/loading.png`} alt="Loading UI" />
...
<ImageCard src={`/blog/supabase-auth-next-13/home-loading.gif`} alt="Home Page loading" />
...
**EDIT:** The new [Next.js Docs](https://nextjs.org) have now been updated. Definitely give them a read if you haven't yet.

Happy coding!

## Reference Materials

- **GitHub:** [github.com/mryechkin/nextjs-supabase-auth](https://github.com/mryechkin/nextjs-supabase-auth) (`v2` branch)
